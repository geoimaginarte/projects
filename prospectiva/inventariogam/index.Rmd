...
resid_p <- st_read(conn, layer="residencial_polygon", query="select fid, fid as id, Nombre, Categoria, Area, st_centroid(geom) as geom from residencial_polygon")

indust <- st_read(conn, layer="industrial_polygon")
indust_p <- st_read(conn, layer="industrial_polygon", query="select fid, fid as id, Nombre, Categoria, Area, st_centroid(geom) as geom from industrial_polygon")

ofic <- st_read(conn, layer="oficentros_polygon")
ofic_p <- st_read(conn, layer="oficentros_polygon", query="select fid, fid as id, Nombre, Categoria, Area, st_centroid(geom) as geom from oficentros_polygon")

comer <- st_read(conn, layer="comercials_point", query="select fid, fid as id, Nombre, Categoria, shop, geom from comercials_point")

rest <- st_read(conn, layer="restaurantes_point", query="select fid, fid as id, Nombre, Categoria, Cadena, Servicios, Cocina, geom from restaurantes_point")
...
map_resid <- leaflet(width = "100%")%>%
                  ...
                  addMarkers(data= shared_resid, group = "Z. Residenciales", label = ~Nombre, icon = resicon)%>% 
                  addLayersControl(
...
map_indust <- leaflet(width = "100%")%>%
                  ...
                  addMarkers(data= shared_indust, group = "Z. Industriales", label = ~Nombre, icon = indicon)%>% 
                  addLayersControl(
                      baseGroups = c("OSM", "Calles en oscuro", "Satelite"),
...
map_ofic <- leaflet(width = "100%")%>%
                  ...
                  addMarkers(data= shared_ofic, group = "Oficentros", label = ~Nombre, icon = oficon)%>% 
                  addLayersControl(
                      baseGroups = c("OSM", "Calles en oscuro", "Satelite"),
...
filter_select("f", "Busque por id",  shared_indust, group = ~id, multiple = T)
```
...
filter_select("fo", "Busque por id",  shared_ofic, group = ~id, multiple = T)
```
