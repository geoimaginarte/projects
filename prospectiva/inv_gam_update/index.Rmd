...
inv_r <- read_excel("E:/Trabajos Fran/Prospectiva investigación de mercado/InventarioGAM/Datos/DATASET_ENINVENTARIO.xlsx", sheet = "Residenciales", range = cell_cols("A:V"))
#inv_r[2:4] <- list(NULL)
#inv_r[3:8] <- list(NULL)
#inv_r[5:6] <- list(NULL)
#inv_r[23] <- NULL
...
resid_rango <- icons(
  iconUrl = ifelse(shared_resid$data()$FECHA < 15,
    "bydate/house_10.png", ifelse(shared_resid$data()$FECHA < 20, "bydate/house_15.png", "bydate/house_NA.png"
  )))

indust_rango <- icons(
  iconUrl = ifelse(shared_indust$data()$Fecha < 15,
    "bydate/indust_10.png", ifelse(shared_indust$data()$Fecha < 20, "bydate/indust_15.png", "bydate/indust_NA.png"
  )))

ofic_rango <- icons(
  iconUrl = ifelse(shared_ofic$data()$Fecha_inicio < 15,
    "bydate/office_10.png", ifelse(shared_ofic$data()$Fecha_inicio < 20, "bydate/office_15.png", ifelse(shared_ofic$data()$Fecha_inicio < 30, "bydate/office_20.png", "bydate/office_NA.png"
  ))))

comer_rango <- icons(
  iconUrl = ifelse(shared_comer$data()$Fecha < 15,
    "bydate/comer_10.png", ifelse(shared_comer$data()$Fecha < 20, "bydate/comer_15.png", "bydate/comer_NA.png"
  )))
...
map_resid <- leaflet(width = "100%")%>%
                  ...
                  addMarkers(data= shared_resid, group = "Z. Residenciales", label = ~Nombre, icon = resid_rango, popup = resipop)%>% 
...
map_indust <- leaflet(width = "100%")%>%
                  ...
                  addMarkers(data= shared_indust, group = "Z. Industriales", label = ~Nombre, icon = indust_rango, popup = indupop)%>% 
                  addLayersControl(position = "topleft",
                      baseGroups = c("OSM", "Calles en oscuro", "Satelite"),
...
map_ofic <- leaflet(width = "100%")%>%
                  ...
                  addPolygons(data=conur, group="Conurbanos GAM", color = "#0F9D58", popup= conurpop, fillOpacity = 0.05)%>%   
                  addPolygons(data=dist, group="Distritos GAM", color = "#698eff", popup= distpop, fillOpacity = 0.05)%>%
                  addMarkers(data= shared_ofic, group = "Oficentros", label = ~Nombre, icon = ofic_rango, popup = ofipop)%>% 
                  addLayersControl(position = "topleft",
                      baseGroups = c("OSM", "Calles en oscuro", "Satelite"),
...
map_comer <- leaflet(width = "100%")%>%
                  ...
                  addPolygons(data=conur, group="Conurbanos GAM", color = "#0F9D58", popup= conurpop, fillOpacity = 0.05)%>%  
                  addPolygons(data=dist, group="Distritos GAM", color = "#698eff", popup= distpop, fillOpacity = 0.05)%>%
                  addMarkers(data= shared_comer, group = "Comerciales", label = ~Nombre, icon = comer_rango, popup = comerpop)%>% 
                  addLayersControl(position = "topleft",
                      baseGroups = c("OSM", "Calles en oscuro", "Satelite"),
