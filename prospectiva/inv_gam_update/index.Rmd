...
ofic_p <- st_centroid(ofic) %>% 
    dplyr::select(fid, Nombre, Categoria, Area) %>% 
...
resid_inv <- dplyr::inner_join(resid_p, inv_r, by = c("fid"="fid"))
indu_inv <- dplyr::inner_join(indust_p, inv_i, by = c("id"="fid"))
ofic_inv <- dplyr::inner_join(ofic_p, inv_o, by = c("id"="fid"))
comer_inv <- dplyr::inner_join(comer, inv_c, by = c("id"="fid"))
rest_inv <- dplyr::inner_join(rest, inv_rt, by = c("fid"="fid"))

rest <- st_intersection(rest_inv, dist)
ofic_inv <- st_intersection(ofic_inv, dist)

#library(writexl)
#write_xlsx(indu_inv, "E:\\Trabajos Fran\\Prospectiva investigación de mercado\\InventarioGAM\\Datos\\otros_industrial\\indust.xlsx")
