...
resid_inv <- dplyr::left_join(resid_p, inv_r, by = c("fid"="ID"))
...
pal_r <- colorNumeric(c("transparent", "#41B6C4", "#0C2C84"), values(resid_den),
...
map_resid <- leaflet(width = "100%")%>%
                  ...
                  addLayersControl(position = "topleft",
                      ...
                      overlayGroups = c( "Z. Residenciales", "Limites de residenciales","Distritos GAM", "Cornubanos GAM"),
...
map_indust <- leaflet(width = "100%")%>%
                  ...
                  addLayersControl(position = "topleft",
                      baseGroups = c("OSM", "Calles en oscuro", "Satelite"),
                      overlayGroups = c( "Z. Industriales", "Limites de Z.Industriales", "Z.I. Planes Reguladores", "Z.I. PlanGAM 2013", "Distritos GAM", "Conurbanos GAM"),
                      options = layersControlOptions(collapsed = FALSE))%>%
...
map_ofic <- leaflet(width = "100%")%>%
                  ...
                  addLayersControl(position = "topleft",
                      baseGroups = c("OSM", "Calles en oscuro", "Satelite"),
                      overlayGroups = c( "Oficentros", "Distritos GAM", "Conurbanos GAM"),
                      options = layersControlOptions(collapsed = FALSE))%>%
                  addMeasure(
                      position = "bottomleft",
                      primaryLengthUnit = "meters",
...
map_rest <- leaflet(width = "100%")%>%
                  ...
                  addPolygons(data=conur, group="Conurbanos GAM", color = "#0F9D58", popup= conurpop, fillOpacity = 0.05)%>%  
                  addPolygons(data=dist, group="Distritos GAM", color = "#698eff", popup= distpop, fillOpacity = 0.05)%>%
                  addRasterImage(resid_den, group = "Densidad residencial por area", colors = pal_r, opacity = 0.8) %>%  
                  addRasterImage(indust_den, group = "Densidad industrial por cant.personas", colors = pal_i, opacity = 0.8) %>%
                  addRasterImage(ofic_den, group = "Densidad residencial por area arrendable", colors = pal_o, opacity = 0.8) %>%
                  ...
                  addLayersControl(position = "topleft",
                      baseGroups = c("OSM", "Calles en oscuro", "Satelite"),
                      overlayGroups = c( "Restaurantes", "Densidad de Restaurantes","Densidad residencial por area","Densidad industrial por cant.personas","Densidad residencial por area arrendable","Distritos GAM", "Conurbanos GAM"),
                      options = layersControlOptions(collapsed = FALSE))%>%
                  hideGroup(c("Densidad residencial por area","Densidad industrial por cant.personas","Densidad residencial por area arrendable")) %>% 
                  addMeasure(
                      position = "bottomleft",
                      primaryLengthUnit = "meters",
                      primaryAreaUnit = "sqmeters",
...
densi_ofic <- densi_ofic %>% 
  plot_ly(
    ...
    frame = ~Fecha_inicio,
    alpha = 0.3)
densi_ofic <- densi_ofic %>% animation_slider(
  currentvalue = list(
    prefix = "Fecha de inicio "
  )
)
densi_ofic <- densi_ofic %>%
  layout(
...
### Mapa de Densidad de Residenciales por magnitud de Area
...
### Mapa de Densidad de Oficentros por magnitud de Area Arrendable segun Fecha de inicio
...
filter_checkbox("auto", "Automatic", shared_rest, ~ifelse(Cadena == "KFC", "KFC", "Otras Cadenas"))
