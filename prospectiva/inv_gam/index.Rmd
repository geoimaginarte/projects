...
comer <- st_read(conn, layer="comercials_point", query="select fid, fid as id, Nombre, Categorias, shop, geom from comercials_point where shop = 'mall'")
...
densi_resid <- plot_ly(
    type = 'densitymapbox',
    lat = st_coordinates(shared_resid$data())[,2],
    lon = st_coordinates(shared_resid$data())[,1],
    z = shared_resid$data()$Area,
    coloraxis = 'coloraxis',
    radius = 10)
...
densi_indust <- plot_ly(
    type = 'densitymapbox',
    lat = st_coordinates(shared_indust$data())[,2],
    lon = st_coordinates(shared_indust$data())[,1],
    z = shared_indust$data()$Cantidad_personas,
    coloraxis = 'coloraxis',
    radius = 10)
...
densi_ofic <- plot_ly(
    type = 'densitymapbox',
    lat = st_coordinates(shared_ofic$data())[,2],
    lon = st_coordinates(shared_ofic$data())[,1],
    z = shared_ofic$data()$Cantidad_personas,
    coloraxis = 'coloraxis',
    radius = 10,
    frame = shared_ofic$data()$Fecha_inicio,
    alpha = 0.3)
...
### Mapa de Densidad por Cantidad de Personas
...
### Mapa de Densidad de Oficentros segun Area Arrendable y Fecha de inicio
