...
Chart_1 <- Turtle %>% ggplot() + 
  geom_line(mapping = aes(x = as.Date(Date, "%Y-%m", tz = "UTC"), y = nharv * 500000/100 ), 
            colour="#ED7D31") + 
  geom_point(mapping = aes(x = as.Date(Date, "%Y-%m", tz = "UTC"), y = nharv * 500000/100), 
             size = 3, shape = 21, fill = "#ED7D31") +
  geom_bar(mapping = aes(x = as.Date(Date, "%Y-%m", tz = "UTC"), y = egglaying), 
           stat = "identity", colour = gray(0.5), fill = "#2F528F", alpha = 0.5) + 
